...
export const createShiprocketAdhocOrder = async (orderData) => {
  const authToken = await getShiprocketToken();
  ...
  try {
    ...
    logMessage(
      "info",
      `Ad-hoc order created successfully: ${JSON.stringify(response.data)}`,
      "shiprocket-info"
    );
    return response.data;
  } catch (error) {
    logMessage(
      "error",
      `Error creating ad-hoc order: ${error?.response?.data || error.message}`,
      "shiprocket-errors"
    );
    throw error;
  }
